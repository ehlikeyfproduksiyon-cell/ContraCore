...
class MainWindow(QMainWindow):
    ...
    def _toggle_btn_style(self, btn, checked, color_on, color_off=None):
        ...
        btn.setStyleSheet(f'''
            QPushButton {{
                background: {"rgba(11,31,58,0.1)" if checked else "#F3F4F6"};
                color: {NAVY if checked else c_off};
                border: 1.5px solid {NAVY if checked else BORDER};
                border-radius: 8px; font-size:11px; font-weight:600;
                padding: 0 12px;
            }}
            QPushButton:hover {{
                background: rgba(11,31,58,0.15); border-color:{NAVY};
            }}
        ''')
